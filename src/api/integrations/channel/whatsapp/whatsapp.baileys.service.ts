...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async sendMessage(
    ...
    if (message['viewOnceMessage']) {
      // Apply contextInfo if provided
      if (contextInfo && message['viewOnceMessage']['message']) {
        if (message['viewOnceMessage']['message']['interactiveMessage']) {
          message['viewOnceMessage']['message']['interactiveMessage']['contextInfo'] = contextInfo;
        } else {
          // For other types of viewOnceMessage
          const messageType = Object.keys(message['viewOnceMessage']['message'])[0];
          if (messageType && message['viewOnceMessage']['message'][messageType]) {
            message['viewOnceMessage']['message'][messageType]['contextInfo'] = contextInfo;
          }
        }
      }
      ...
      viewOnceMessage: {
        message: {
          interactiveMessage: {
            ...
            header: { title: data.title, subtitle: data.description, hasMediaAttachment: false },
            nativeFlowMessage: {
              buttons: [
                {
                  name: 'single_select',
  ...
  public async reactionMessage(data: SendReactionDto) {
    return await this.sendMessageWithTyping(data.key.remoteJid, {
      react: { key: data.key, text: data.reaction },
    });
  }
